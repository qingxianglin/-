...
public class TableDemo9 extends DemoModule {
    Object[][] data= null;
    ...
    class PageList{
        ...
        public Object[][] findData(int currentPage, int pageSize){
            int totalRowCount = parkingLotDBUtils.queryAuthReqCount2("等待审核");
            if(currentPage<1){
                currentPage=1;
            }
            ...
            List<AuthReqBean> temp = parkingLotDBUtils.queryAuthReqBean(null,startIndex,endIndex);
            ...
            for(int i = startIndex;i<endIndex;++i){
                ...
                tempList[index] = new Object[]{tempbean.getRequestId(),tempbean.getUserName(),tempbean.getUserId(),tempbean.getRoleNameZh(),
                        tempbean.getRoleNameEn(),tempbean.getRoleId(),"等待审核"};
                ++index;
            }
    ...
    public TableDemo9(final JTabbedPane parentTabbedPanel) {
    ...
    public JScrollPane createTable() {
        ...
        comboBox.addItem("等待审核");
        comboBox.addItem("审核通过");
        comboBox.addItem("审核不通过");
        comboBox.addItemListener(new ItemListener() {
            @Override
            public void itemStateChanged(ItemEvent e) {
                comboBox.show();
                String itemString = e.getItem().toString();
                if(itemString.equals("等待审核")){
                    return;
                }
                AuthReqBean authReqBean  = new AuthReqBean();
                authReqBean.setRequestId(Integer.parseInt(tableView.getValueAt(tableView.getSelectedRow(),0).toString()));
                authReqBean.setStatus(itemString);
                parkingLotDBUtils.updateAuthReqBean(authReqBean);
                comboBox.hide();
                System.out.println(tableView.getValueAt(tableView.getSelectedRow(),0));
                pageList.setTotalCount();
            }
        });
